...
export default class Calendar {
   ...
   generateMonthDataForCalendar(year, month) {
      ...
      while (countWeek <= this.weekInMonth) {
         ...
         while (countDays <= this.daysInWeek) {
            ...
            const currentDate = new Date(year, month, currentDay).toDateString();
            let dateString = year + '-' + (month < 10 ? '0' : '') + month + '-' + (currentDay < 10 ? '0' : '') + currentDay;
            let day = {
               dayId: countDays,
               value: null,
               isActive: false,
               dateString: dateString,
               events: this.eventsMarkerArray.filter((item: CalendarMarkerEvent) => item.markedDay === dateString),
            };
   ...
   generateCurrentMonthDataForCalendar() {
      return this.generateMonthDataForCalendar(this.currentYear, this.currentMonth);
   }
   generateYearDataForCalendar(year) {
      const yearData = {};
      let currentMonth = 0;
      while (currentMonth <= this.monthInYear) {
         const [monthArray, isActive] = this.generateMonthDataForCalendar(year, currentMonth);
         const month = {
   ...
   getDayNames() {
      const daysInWeek = [{ name: 'Dim' }, { name: 'Lun' }, { name: 'Mar' }, { name: 'Mer' }, { name: 'Jeu' }, { name: 'Ven' }, { name: 'Sam' }];
