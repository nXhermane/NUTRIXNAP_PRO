...
const GestureSelector = (props: Props) => {
    ...
    const selectorAnimatedStyle = useAnimatedStyle(() => ({
        transform: [
            { translateY: selectorOffset.value.y },
            { translateX: selectorOffset.value.x }
        ],
    ...
    const gesture = Gesture.Pan()
        ...
        .onUpdate(e => {
            ...
            const stepX = Math.floor(
                (e.translationX + sStart.value.x + ecartX) / ecartX
            );
            ...
            if (
                sOffset.value.y >= 0 &&
                sOffset.value.y <= cellHeight.value * 24 - sHeight.value
            ) {
                sOffset.value = {
                    y: step * ecart,
                    x: stepX * ecartX
    ...
    const pointTopGesture = Gesture.Pan()
        ...
        .onUpdate(e => {
            ...
            const step = Math.floor(
                (-e.translationY + sHeightStart.value) / ecart
            );

            sHeight.value =
                step * ecart || -e.translationY + sHeightStart.value;
            sOffset.value = {
