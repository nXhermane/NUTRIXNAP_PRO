...
const HourMark = (props: Props) => {
    ...
    const graduiatorAnimatedStyle = useAnimatedStyle(() => ({
        transform: [
            {
                translateY:
                    props.offset.value.y +
                    (props.add ? props.add.value : 0) -
                    size.s2
            }
        ]
    }));
    ...
    return (
        <Animated.View style={[style.graduiator, graduiatorAnimatedStyle]}>
            {isActive && <Text style={style.textHour}>{Hour}</Text>}
        </Animated.View>
    );
};
