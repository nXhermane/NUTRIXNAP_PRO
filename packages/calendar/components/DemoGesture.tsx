...
const GestureSelector = (props: Props) => {
   ...
   const selectorAnimatedStyle = useAnimatedStyle(() => ({
      transform: [{ translateY: selectorOffset.value.y - selectorHeight.value * 1 }, { translateX: selectorOffset.value.x - selectorWidth * -1 }],
      height: selectorHeight.value,
      backgroundColor: selectorIsPressed.value ? colors.blue + '20' : 'transparent',
   }));
   const gesture = Gesture.Pan()
      ...
      .onUpdate((e) => {
         ...
         const step = Math.floor((e.translationY + selectorStart.value.y) / ecart);
         const ecartX = selectorWidth;
         const stepX = Math.floor((e.translationX + selectorStart.value.x + ecartX) / ecartX);
         ...
         if (selectorOffset.value.y >= 0 && selectorOffset.value.y <= cellHeight.value * 24 - selectorHeight.value) {
   ...
   const pointTopGesture = Gesture.Pan()
      .onBegin(() => {
         selectorTopDist.value = selectorOffset.value.y + selectorHeight.value;
      })
      .onUpdate((e) => {
         const ecart = cellHeight.value / 4;
         const step = Math.floor((-e.translationY + selectorHeightStart.value) / ecart);

         selectorHeight.value = step * ecart || -e.translationY + selectorHeightStart.value;
         selectorOffset.value = {
   ...
   return (
      <GestureDetector gesture={gesture}>
         <Animated.View style={[style.selector(selectorWidth), selectorAnimatedStyle]} collapsable={false}>
            <Text style={{ color: 'white', alignSelf: 'center' }}>Demo Gesture</Text>
