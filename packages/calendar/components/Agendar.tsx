...
const Agendar = (props: Props) => {
   ...
   const [scrollViewVisibleContentWidth, setScrollViewVisibleContentWidth] = useState(0);
   const scrollViewVisibleContentHeight = useDerivedValue(() => cellHeight.value * 24);
                     ...
                     <View style={style.agendarHeaderDays}>
                        {item.days.slice(0, format).map((day) => (
                           <Pressable style={[style.dayCell]}>
                              <Text style={style.dayCellTitle}>{days[day.dayId].name}</Text>
                              <View style={style.dayCellValueContainer}>
                                 <Text style={style.dayCellValue}>{day.value}</Text>
                              </View>
                           </Pressable>
                        ))}
                     </View>
         ...
         <ScrollView>
            <View style={style.agendarBody}>
               <Animated.View style={[style.agendarBodyLateral]}>
                  ...
                  <HourMark offset={selectorOffset} cell={cellHeight} isActive />
                  <HourMark offset={selectorOffset} cell={cellHeight} add={selectorHeight} />
               </Animated.View>
               <View>
                  <GestureDetector gesture={tapGesture}>
                     ...
                     >
                        {Month.map((item, n) => {
                           return (
                              <View key={item.weekId + '' + n} style={[style.agendarBodyGuide]} onLayout={handleLayout}>
                                 {Array(format)
                                    .fill(0)
                                    .map((i, index) => (
                                       <View
                                          onLayout={(e) => {
                                             setSelectorWidth(e.nativeEvent.layout.width);
                                          }}
                                       ...
                                       >
                                          {day.map((item, u) => (
                                             <AnimatedPressable
                                                style={[style.dayRow, cellAnimatedStyle]}
                                                key={String(u)}
                                                onPress={() => setSelectorIsVisible(true)}
                                             ></AnimatedPressable>
                                          ))}
                                       </View>
                                    ))}
                        ...
                        {selectorIsVisible && (
                           <DynamicGestorSelectorManager
                              ...
                              scrollViewVisibleContentWidth={scrollViewVisibleContentWidth}
                              format={format}
                           />
                        )}
                     </Animated.ScrollView>
