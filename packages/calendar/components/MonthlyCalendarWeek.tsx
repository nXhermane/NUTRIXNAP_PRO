...
const MonthlyCalendarWeek = ({ weekData, currentCalendarInfo, setCurrentCalendarInfo, onPressDay, setActiveDayName }: Props) => {
   ...
   return (
      <View style={style.calendarBodyWeek}>
         {weekData.days.map((day, index) => {
            ...
            const isSelect = day.value === currentCalendarInfo.selectDay.value;
            return (
                           ...
                           {!day.isActive && !isSelect && (
                              <View style={style.eventDotContainer}>
                                 {day.events.slice(0, 3).map((event, index) => (
                                    <View key={String(index)} style={style.eventDot(event.color)} />
                                 ))}
                              </View>
                           )}
                        </>
...
const styles = ({ colors, size }) =>
   StyleSheet.create({
      ...
      daysPressable: ({ isActive, isSelect }) => ({
         backgroundColor: isActive ? colors.tangerine : isSelect ? colors.yell_gre : 'transparent',
         width: size.s7,
         height: size.s7,
         borderRadius: 500,
      ...
      daysNumberStyle: ({ isActive, isSelect }) => ({
         fontFamily: 'inter',
         fontSize: size.s3,
         color: isActive ? colors.text.primary : isSelect ? colors.blue : colors.text.primary,
         fontWeight: '700',
      }),
      cell: {
         width: size.s10,
