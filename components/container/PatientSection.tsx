...
const PatientSection: React.FC<Props> = ({
   ...
   return (
      <View style={style.serviceContainer}>
            ...
            <View style={style.serviceHead}>
               ...
               {withRight && (
                  <View style={style.sectionHeaderRight}>
                     {withAddBtn && (
                        <Pressable onPress={(e: PressEvent) => onPressAddBtn(e)}>
                           {({ pressed }) => (
                              <Ionicons
                                 name={'add-outline'}
                                 color={pressed ? colors.purple300 : colors.black300}
                     ...
                     {withSearch && (
                        <Pressable onPress={onPressSearch}>
                           {({ pressed }) => (
                              <Ionicons name={'search'} color={pressed ? colors.purple300 : colors.black300} size={pressed ? size.s4 : size.s5} />
                           )}
                        </Pressable>
                     )}
                     {withFilter && (
         ...
         {body && <View style={style.bodyContainer}>{<Text style={style.body(centerBody)}>{body}</Text>}</View>}
         {children && <View style={[style.serviceInner, contentContainerStyle]}>{children}</View>}
      </View>
   );
};
