...
const FoodDiaryForm = ({ onClose, patientUniqueId, foodDiaryData, isUpdate = false }: Props) => {
   ...
   const { values, onSubmit, onChange, errors, isSubmitting, isValid } = useFoodDiaryForm(patientUniqueId, onClose, foodDiaryData, isUpdate);
                  ...
                  <BottomSheetView style={style.container}>
                     <View style={style.foodDiaryFormHeader}>
                        <Text style={style.foodDiaryFormTitle}>
                           Journal <Text style={{ color: colors.blue300 }}>Alimentaire</Text>
                        </Text>
                        <Text style={style.foodDiaryFormSubTitle}>
                           {!isUpdate ? 'Emregistrer' : 'Consultez'} le journal alimentaire de votre patient
                        </Text>
                     </View>
                     <KeyboardAwareScrollView>
                        <FoodDiaryFormBody onChangeValue={onChange} values={values} />
                        <View style={style.formBtn}>
                           <Button
                              r={size.s3}
                              title={!isUpdate ? 'Enregistrer' : 'Mettre à jour'}
                              h={size.s10}
                              w={size.width * 0.85}
                              onPress={onSubmit}
                              st={{
