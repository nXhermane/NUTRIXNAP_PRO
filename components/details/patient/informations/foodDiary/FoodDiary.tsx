...
const FoodDiary = ({ patientUniqueId }: Props) => {
   ...
   useEffect(() => {
      if (patientUniqueId != undefined)
         core.foodDiaryS.getFoodDiaryByPatientUniqueId(patientUniqueId).then((foodDiaries: FoodDiaryDTO[]) => {
            const data = foodDiaries.filter((item, index) => index > foodDiaries.length - 4);
               ...
               return (
                  <FoodDiaryItem
                     ...
                     onDelete={() => {
                        ...
                        Alert.confirm(`Voulez-vous vraiment supprimer le journal Alimentaire  ajouté le ${inter[0]} à ${inter[1]} ?`).then(
                           (check: boolean) => {
                              if (check) {
                                 core.foodDiaryS.deleteFoodDiary(data.id).then(() => {
                                    forceUpdate();
                                 });
                              }
                           },
