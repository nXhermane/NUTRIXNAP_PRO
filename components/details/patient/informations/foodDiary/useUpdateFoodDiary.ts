...
export default function useUpdateFoodDiary(patientUniqueId: number): (
   ...
   const handleUpdateProcess = async (
      ...
      const imagesChanged = JSON.stringify(mappedImage) != JSON.stringify(foodDiaryData.images);
      const foodDiary = {
         id: foodDiaryData.id,
      ...
      return new Promise<boolean>((resolve, reject) => {
         Alert.confirm('Voulez-vous vraiment mettre à jour ce journal ?').then((check: boolean) => {
            if (check) {
               core.foodDiaryS.updateFoodDiary(foodDiary).then(({ id }) => {
                  if (imagesChanged) {
