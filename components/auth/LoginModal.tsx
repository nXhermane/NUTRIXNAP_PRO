import { StyleSheet, Text, View, Pressable, Modal, KeyboardAvoidingView, PressEvent } from 'react-native';
...
const LoginModal = (props: Props) => {
   ...
   const modalAnimatedStyle = useAnimatedStyle(() => {
      return {
         ...
         borderRadius: interpolate(modalSize.value, [0, 0.5, 1], [1000, 4, 0]),
   ...
   const animation = (value: number) => {
      if (value === 1) {
         ...
         position.value = withTiming({ x: props.animatedData.x, y: props.animatedData.y }, { duration: 500 }, () => {
   ...
   const onSubmit = (e: PressEvent) => {
      core.userS.createUser(formInfo).then((user) => {
         ...
         imageDownloadFunc(formInfo.profil_img, 'users_' + formInfo.name.split(' ').join('').toLowerCase(), 'profil_img.' + pickType).then(
            ({ uri }) => {
               core.userS.updateUser({ id: user.id, profil_img: uri }).then((upUser) => {
                  core.setUser(upUser);
                  console.log(upUser);
                  animation(0);
         ...
         <Animated.View style={[modalAnimatedStyle]}>
            <View style={style.loginFormContainer}>
               <View style={style.header}>
                  <Text style={style.formTitle}>Mise à jour du profil</Text>
               </View>
                  ...
                  <KeyboardAwareScrollView>
                     <View style={style.profilImgContainer}>
                        <Avatars
                           ...
                           st={{
                              alignSelf: 'center',
                              borderWidth: formInfo.profil_img === '' ? size.s1 : 0,
                     ...
                     <View style={style.formInputContainer}>
                        ...
                        <SelectionInput
                           ...
                           value={SEXEDATA.find((item) => item.code === formInfo.gender)?.label}
                           selectedId={1}
                        ...
                        <SelectionInput
                           ...
                           data={COUNTRYDATA}
                           custormItem={CountryItem}
                           value={getEmojiFlag(formInfo.country) + '  ' + getCountryData(formInfo.country)?.name}
                           selectedId={1}
                           onChange={(ids: number, data: any) => {
                              dispatch({
                                 type: 'country',
                        ...
                        <TelInput
                           label={'Portable'}
                           value={formInfo.tel}
                           tel={'+' + getCountryData(formInfo.country)?.phone}
