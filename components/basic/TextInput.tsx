...
export interface Props {
   ...
   inputMode?: 'numeric' | 'decimal' | 'none' | 'tel' | 'url' | 'email' | 'text';
   isPassword?: boolean;
   placeholder?: string;
   h?: number;
   r?: number;
...
const TextInput = (props: Props) => {
   ...
   return (
      <View style={[style.inputContainer, st]}>
         ...
         >
            {icon && <View style={style.iconContainer}>{icon(colors.black100, size.s5)}</View>}
            <RTtextInput
               style={[
                  style.textInput,
                  props.multiline && {
            ...
            />
            {isPassword && (
               <Pressable style={style.rightIconContainer} onPress={() => setPasswordIsVisible((prev) => !prev)}>
                  <Ionicons name={passwordIsVisible ? 'eye' : 'eye-off'} size={size.s5} color={colors.gray300} />
               </Pressable>
            )}
            {rightIcon && (
               <Pressable
