...
interface Props extends Omit<TextInputProps, 'label' | 'value' | 'icon' | 'placeholder'> {
   label: string;
   isRequire?: boolean;
   value: string;
   ...
   return (
      <View style={style.selectionInputContainer}>
         <View style={style.labelContainer}>
            {label && <Text style={[style.label, props.labelStyle]}>{label}</Text>}
            {isRequire && (
         ...
         <View style={style.datetimeInputContainer}>
            <TextInput
               ...
               onPress={() => setDisplayDatePopup((prev: boolean) => !prev)}
               rightIcon={(color, size) => <Ionicons name={displayDatePopup ? 'chevron-up' : 'chevron-down'} color={colors.black200} />}
               r={r && r}
               icon={(color, size) => <Ionicons name={'calendar-outline'} color={colors.gray300} size={size} />}
               st={{
                  width: '60%',
                  paddingHorizontal: 0,
               }}
            ...
            <TextInput
               isRequire={isRequire}
               value={dateTime.toLocaleTimeString().split(':').slice(0, -1).join(':')}
               editable={false}
               placeholder={timePlaceholder}
               onPress={() => setDisplayTimePopup((prev: boolean) => !prev)}
               rightIcon={(color, size) => <Ionicons name={displayTimePopup ? 'chevron-up' : 'chevron-down'} color={colors.black200} />}
               r={r && r}
               st={{
                  paddingHorizontal: 0,
                  width: '40%',
               }}
               icon={(color, size) => <SimpleLineIcons name={'clock'} color={colors.gray300} size={size} />}
            />
         </View>
         {displayDatePopup && (
            <DateTimePicker
