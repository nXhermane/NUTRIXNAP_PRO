...
const ImagePicker = (props: React.PropsWithChildren<ImagePickerProps>) => {
    ...
    const [selectedImage, setSelectedImage] =
        React.useState<ImagePickerItemType[]>(value);
                    ...
                    if (selectedImage.length < numberOfImages) {
                        picker().then(images => {
                            images.forEach((image, index) => {
                                const rest =
                                    numberOfImages - selectedImage.length;
                                if (rest > index) {
                                    setSelectedImage(prev => [
                                        ...prev,
                                        { ...image, id: prev.length + 1 }
                                    ]);
                                }
                            });
                        });
                ...
                {!children ? (
                    <View style={style.imagePickerZone}>
                        <View style={style.imagePickerInnerZone}>
                            ...
                            />
                            {placeholder && selectedImage.length === 0 && (
