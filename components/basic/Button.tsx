import { StyleSheet, Text, View, TouchableHighlight, ViewStyle, TextStyle, PressEvent } from 'react-native';
...
const Button: React.FC<ButtonProps> = (props) => {
   ...
   const { title = '', gradient = false, outlined = false, onPress = (e: PressEvent) => {}, children } = props;
   const styles = (theme: ThemeInterface) =>
      StyleSheet.create({
         btnContainer: {
            ...
            alignItems: 'center',
            overflow: 'hidden',
            padding: outlined ? (props.disableBorder ? 0 : theme.size.s1 / 2) : 0,
            backgroundColor: gradient ? 'transparent' : props.c || theme.colors.blue,
            ...props.sc,
         },
         btnContainerHight: {
            flex: 1,
         ...
         btnSectionContainer: {
            ...
            alignItems: 'center',
            borderRadius: props.r || theme.size.s8 - theme.size.s1 / 2,
            backgroundColor: outlined ? props.outlinedBgColor || theme.colors.w : 'transparent',
         ...
         btnText: {
            color: !outlined ? props.fc || theme.colors.white : props.fc || theme.colors.blue,
   ...
   const animateWidthStyle = useAnimatedStyle(() => ({
      width: interpolate(width.value, [0, 1], [props.w || theme.size.s100 * 3, props.w - theme.size.s5 || theme.size.s100 * 3 - theme.size.s5]),
   ...
   const ButtonContainer: React.FC<{
      ...
      return (
         <>
            {gradient ? (
               <LinearGradient
                  colors={[theme.colors.blue, 'rgba(0,145,255,1)', 'rgba(17,199,255,1)']}
                  start={[0.35, 0]}
                  end={[1, 0.65]}
                     ...
                     <View style={style.btnSectionContainer}>
                        {props.iconStart && (
                           <View style={style.iconContainer}>
                              {props.iconStart({
                                 style: {
                                    color: outlined ? theme.colors.blue : theme.colors.white,
                                    size: theme.size.s6,
                                 },
                              })}
                           </View>
