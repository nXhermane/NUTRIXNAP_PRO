...
interface Props {
   ...
   onChange: (ids: number | string | number[] | string[], data: Option | Option[]) => void;
   withSearch?: boolean;
...
const SelectionInput = (props: Props) => {
   ...
   const index = data.length <= 5 ? 0 : data.length <= 7 ? 1 : data.length <= 10 ? 2 : data.length <= 15 ? 3 : 4;
   // callbacks
   ...
   return (
      <View style={style.selectionInputContainer}>
         <TextInput
            ...
            rightIcon={(color, size) => <Ionicons name={displayPopup ? 'chevron-up' : 'chevron-down'} color={color} />}
            onPress={() => {
         ...
         {displayPopup && (
            ...
            >
               <Pressable style={style.modalContainer} onPress={() => setDisplayPopup((prev: boolean) => false)}>
                  <BottomSheetModalProvider>
                     <BottomSheetModal
                        ...
                        onDismiss={() => setDisplayPopup((prev: boolean) => false)}
                     >
                        <GroupSelection
                           ...
                           optionLabel={
                              <View style={style.searchContainer}>
                                 {withSearch && (
                                    <SearchInput
                                       value={searchValue}
                                       onChange={(value: string) => setSearchValue()}
