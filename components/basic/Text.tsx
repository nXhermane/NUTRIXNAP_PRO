...
const Text = React.forwardRef((props: React.PropsWithChildren<Props>, ref) => {
   const { type = 'h1', style, color, font, size, upper = 'none', align = 'l', ...otherProps } = props;
   ...
   return (
      <RNText
         ...
         style={Style.text(
            ...
            {
               textTransform: upper === true ? 'uppercase' : upper === false ? 'lowercase' : upper,
...
const styles = ({ colors, size }: ThemeInterface) =>
   StyleSheet.create({
      text: (typo: { size: number; color: string; font: string }, style: TextStyle = {}) => ({
