...
const SearchBtn = ({ placeholder, searchPageLink }: Props) => {
   ...
   return (
      <Link href={searchPageLink || 'search/search'} asChild>
         <TouchableHighlight style={style.seachBtn} underlayColor={colors.blue100}>
            <>
               <Ionicons name="search" size={size.s6} color={colors.black100} />
               <Text style={style.textSeachBtn}>{placeholder || 'Reachercher...'}</Text>
            </>
         </TouchableHighlight>
      </Link>
   );
