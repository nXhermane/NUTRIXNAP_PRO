...
import { continents, countries, languages, getCountryCode, getCountryData, getCountryDataList, getEmojiFlag } from 'countries-list';
...
const PatientForm = (props: Props) => {
   ...
   const modalAnimatedStyle = useAnimatedStyle(() => {
      return {
         ...
         height: interpolate(modalSize.value, [0, 0.5, 1], [0, size.width, size.height]),
   ...
   const animation = (value: number) => {
      if (value === 1) {
         ...
         position.value = withTiming(props.animatedData, { duration: 600 }, () => {
   ...
   const onSubmit = (values) => {
      if (isUpdate) {
         Alert.confirm('Voulez-vous vraiment mettre à jour ce patient ?').then((check: boolean) => {
            if (check) {
               core.patientS.updatePatient(values).then((patient) => {
                  ...
                  existType = pickType.trim() === '' ? existType : pickType;
                  if (pickUri != '') {
                     copyImage(values.profil_img, 'patients_' + patient.unique_id, 'profil_img.' + existType).then(({ uri: copyUri }) => {
                        core.patientS
                           .updatePatient({
                              id: patient.id,
                              profil_img: copyUri,
         ...
         core.patientS.createPatient(values).then((patient) => {
            if (pickUri != '') {
               copyImage(values.profil_img, 'patients_' + patient.unique_id, 'profil_img.' + pickType).then(({ uri: copyUri }) => {
                  core.patientS
                     .updatePatient({
                        id: patient.id,
                        profil_img: copyUri,
   ...
   const { values, touched, handleSubmit, handleChange, errors, isSubmitting, isValid } = formik;
   React.useEffect(() => {
      animation(1);
   });
         ...
         <Animated.View style={[modalAnimatedStyle]}>
            <View style={style.loginFormContainer}>
               <View style={style.submitBtn}>
                  ...
                  >
                     <Ionicons name="save" size={size.s5} color={colors.blue100} />
                  </Pressable>
               </View>
               {displayHeader && (
                  ...
                  {displayHeader && (
                     <View style={style.profilImgContainer}>
                        <Avatars
                           ...
                           st={{
                              alignSelf: 'center',
                              borderWidth: values.image === '' ? size.s1 : 0,
                  ...
                  <KeyboardAwareScrollView>
                     <View style={style.formInputContainer}>
                        ...
                        <SelectionInput
                           ...
                           unique
                           data={SEXEDATA}
                           value={SEXEDATA.find((item) => item.code === values.gender)?.label}
                           selectedId={SEXEDATA.find((item) => item.code === values.gender)?.id}
                        ...
                        <SelectionInput
                           ...
                           value={getEmojiFlag(values.country) + '  ' + getCountryData(values.country)?.name}
                           selectedId={COUNTRYDATA.findIndex((item) => item.code === values.country)}
                           onChange={(ids: number, data: any) => {
                        ...
                        <SelectionInput
                           ...
                           selectedId={1}
                           onChange={(ids: number, data: any) => {
                              handleChange('consultationLocation')(data.label);
                           }}
                           isRequire
                        />
                        <TelInput
                           ...
                           value={values.tel}
                           tel={'+' + getCountryData(values.country)?.phone}
