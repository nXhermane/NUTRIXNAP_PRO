...
export default class FoodDiaryRepository implements IFoodDiaryRepository {
    ...
    private async init() {
        try {
            const hasUsersTable = await this.knex?.schema.hasTable(
                FoodDiaryRepository.tableName
            );
            if (!hasUsersTable) {
                await this.createTable();
                console.log(
                    `Table "${FoodDiaryRepository.tableName}" created successfully.`
                );
            } else {
                console.log(
                    `Table "${FoodDiaryRepository.tableName}" already exists.`
                );
            }
        } catch (error) {
            console.error(
                `Error initializing "${FoodDiaryRepository.tableName}" table:`,
                error
            );
        }
    }
    ...
    async create(foodDiary: CreateFoodDiaryType): Promise<number | null> {
        try {
            const [{ id }] = await this.knex!<FoodDiaryEntity>(
                FoodDiaryRepository.tableName
            )
                ?.insert({
    ...
    async findById(id: number): Promise<FoodDiaryEntity | null> {
        try {
            const foodDiary = await this.knex!<FoodDiaryEntity>(
                FoodDiaryRepository.tableName
            )
                ?.select()
                .where("id", id)
    ...
    async findByPatientUniqueId(
        patient_unique_id: string
    ): Promise<FoodDiaryEntity[]> {
        try {
            const foodDiary = await this.knex!<FoodDiaryEntity>(
                FoodDiaryRepository.tableName
            )
    ...
    async findAll(): Promise<FoodDiaryEntity[]> {
        try {
            const foodDiaries = await this.knex!<FoodDiaryEntity>(
                FoodDiaryRepository.tableName
            )?.select();
            return foodDiaries;
        } catch (error) {
