...
export default class FoodDiaryService implements IFoodDiaryService {
    ...
    async createFoodDiary(foodDiary: FoodDiaryDTO): Promise<FoodDiaryDTO> {
        ...
        const foodDiaryEntity = (await this.repository.findById(
            id as number
        )) as FoodDiaryEntity;
    ...
    async getFoodDiaryById(id: number): Promise<FoodDiaryDTO | null> {
        const foodDiary = (await this.repository.findById(
            id
        )) as FoodDiaryEntity;
        return this.mapper.mapEntityToDto(foodDiary);
    }

    async getFoodDiaryByPatientUniqueId(
        unique_id: string
    ): Promise<FoodDiaryDTO[]> {
        const foodDiaries = (await this.repository.findByPatientUniqueId(
            unique_id
        )) as FoodDiaryEntity[];
        const foodDairiesDto = foodDiaries.map((entity: FoodDiaryEntity) =>
            this.mapper.mapEntityToDto(entity)
        );
        return foodDairiesDto;
    }

    async updateFoodDiary(
        foodDiary: UpdateFoodDiaryDto
    ): Promise<FoodDiaryDTO> {
