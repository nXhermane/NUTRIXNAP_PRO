...
export default class UserService implements IPatientService {
   ...
   async searchPatient(searchValue: string, options?: SearchPatientOptions): Promise<PatientEntity[]> {
      const patients = await this.repository.search(searchValue, options || ({} as SearchPatientOptions));
      return patients;
   }
}
