...
export class MedicalRecordMapper implements Mapper<MedicalRecord, MedicalRecordPersistenceType, MedicalRecordDto> {
   toPersistence(entity: MedicalRecord): MedicalRecordPersistenceType {
      ...
      const medicalRecord = {
         ...
         status: "Active",
         eatingBehaviors: JSON.stringify(
            medicalRecordProps.eatingBehaviors.map((eatBeh: EatingBehavior) => ({
   ...
   toDomain(record: MedicalRecordPersistenceType): MedicalRecord {
      const { medicalRecord, foodDiaries, consultationInformation, foodStory, medicalStory, personalAndSocialStory, patientMeasurement, objectives } =
         record;
   ...
   private foodDiaryToDomain(foodDiary: FoodDiaryPersistenceType): FoodDiary {
      ...
      const foodItems = mealData.foodItems.map(
         (item: any) =>
            new FoodDiaryFoodItem({
               foodId: item.foodId,
   ...
   private patientMeasurementToDomain(patientMeasurement: PatientMeasurementPersistenceType): PatientMeasurements {
      const anthropometricMeasurementsData = JSON.parse(patientMeasurement.anthropometricMeasurements) as (Omit<
         IAnthropometricMeasurement,
         "date"
      > & { date: string })[];
      ...
      const bodyCompositionMeasurementsData = JSON.parse(patientMeasurement.bodyCompositionMeasurements) as (Omit<
         IBodyCompositionMeasurement,
         "date"
      > & { date: string })[];
      ...
      const medicalAnalysisResultsData = JSON.parse(patientMeasurement.medicalAnalysisResults) as (Omit<IMedicalAnalysisResult, "date"> & {
         date: string;
      })[];
   ...
   private eatingBehaviorsToDomain(eatingBehaviors: string): EatingBehavior[] {
      ...
      return eatingBehaviorData.map(
         (eatBeh: { date: string; eatingBehavior: string }) =>
            new EatingBehavior({
   ...
   private consultationInformationToPersistence(
   ...
   private personalAndSocialStoryToPersistence(
   ...
   private objectiveToPersistence(objective: Objective, medicalRecordId: AggregateID): ObjectivePersistenceType {
      return {
         ...
         timeframe: JSON.stringify({
            start: objective.timeframe.start.toString(),
            end: objective.timeframe.end.toString(),
         }),
         body: JSON.stringify(
            objective.isMeasure()
               ? {
                    measureTypeId: objective.measureTypeId,
   ...
   private patientMeasurementToPersistence(patientMeasurement: PatientMeasurements, medicalRecordId: AggregateID): PatientMeasurementPersistenceType {
      return {
         ...
         anthropometricMeasurements: JSON.stringify(
            patientMeasurement.anthropometricMeasurements.map((anth: IAnthropometricMeasurement) => ({
         ...
         bodyCompositionMeasurements: JSON.stringify(
            patientMeasurement.bodyCompositionMeasurements.map((body: IBodyCompositionMeasurement) => ({
         ...
         medicalAnalysisResults: JSON.stringify(
            patientMeasurement.medicalAnalysisResults.map((meAnRe: IMedicalAnalysisResult) => ({
   ...
   private eatingBehaviorsToResponse(eatingBehaviors: EatingBehavior[]): EatingBehaviorDto[] {
      return eatingBehaviors.map((eatBeh: EatingBehavior) => ({
         date: eatBeh.date,
         eatingBehavior: eatBeh.eatingBehavior,
      }));
   }
   private objectiveToResponse(objective: Objective): ObjectiveDto {
      return {
         ...
         body: objective.isMeasure()
            ? {
                 measureTypeId: objective.measureTypeId as string,
                 value: objective.value as number,
                 description: objective.description,
              }
            : { description: objective.description },
         status: objective.status,
   ...
   private patientMeasurementToResponse(patientMeasurement: PatientMeasurements): PatientMeasurementDto {
      return {
         anthropometricMeasurements: patientMeasurement.anthropometricMeasurements.map((anthM: IAnthropometricMeasurement) => ({
            date: anthM.date.toString(),
            measureTypeId: anthM.measureTypeId,
            value: anthM.value,
            unit: anthM.unit,
         })) as AnthropometricMeasurementDto[],
         bodyCompositionMeasurements: patientMeasurement.bodyCompositionMeasurements.map((bodyM: IBodyCompositionMeasurement) => ({
