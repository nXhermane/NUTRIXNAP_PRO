import {
    INVALID_INGREDIENT_LIST,
    INVALID_PREPARATION_LIST,
...
export class Recipe extends AggregateRoot<IRecipe> {
    ...
    get ingredients(): IIngredient[] {
        return this.props.ingredients.map((ingredient: Ingredient) =>
            ingredient.unpack()
        );
    }
    get preparationMethod(): IPreparationStep[] {
        return this.props.preparationMethod.map(
            (prepationStep: PreparationStep) => prepationStep.unpack()
        );
    }
    ...
    removeIngredient(ingredient: Ingredient): void {
        const index = this.props.ingredients.findIndex(
            (ing: Ingredient) => ing.foodId === ingredient.foodId
        );
        if (index !== -1) {
    ...
    removePreparationStep(step: PreparationStep): void {
        const index = this.props.preparationMethod.findIndex(
            (prepStep: PreparationStep) =>
                prepStep.stepNumber === step.stepNumber
        );
        if (index !== -1) {
