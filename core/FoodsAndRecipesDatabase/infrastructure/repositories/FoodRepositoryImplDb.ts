...
export class FoodRepositoryImplDb implements FoodRepository {
   ...
   async getFoodById(foodId: AggregateID): Promise<Food> {
      try {
         const food = await this._getAllFoodInfoQuery().where('fn.foodId', foodId).first();
         if (!food) {
            throw new FoodRepositoryNotFoundException('Food not found', new Error('Food Not Exist'), {
               foodId,
            });
         }
         return this.mapper.toDomain(food);
      } catch (error) {
         throw new FoodRepositoryError('Failed to retrieve food by ID', error as Error, { foodId });
      }
   }

   async getFoodByFoodGroupId(foodGroupId: string, paginated?: Paginated): Promise<Food[]> {
      try {
         const query = this._getAllFoodInfoQuery().where('fg.groupId', foodGroupId);
         ...
         return foodRaws.map((food: FoodPersistenceType) => this.mapper.toDomain(food));
      } catch (error) {
         throw new FoodRepositoryError('Failed to retrieve food by food group ID', error as Error, { foodGroupId, paginated });
      }
   }
   ...
   async getAllFood(
      ...
      try {
         ...
         if (!results || results.length === 0) {
            throw new FoodRepositoryNotFoundException('No food found', new Error(''), {
               foodOrigin,
               paginated,
            });
         }
         return results.map((food: FoodPersistenceType) => this.mapper.toDomain(food));
      } catch (error) {
         throw new FoodRepositoryError('Failed to retrieve all food', error as Error, {
            foodOrigin,
            paginated,
         });
      }
   ...
   async getAllFoodId(foodOrigin?: string): Promise<AggregateID[]> {
      try {
         ...
         if (!foodIds) {
            throw new FoodRepositoryNotFoundException('No food ID found', new Error(''), {
               foodOrigin,
            });
         }
         return foodIds.map(({ foodId }: { foodId: number }) => foodId);
      } catch (error) {
         throw new FoodRepositoryError('Failed to retrieve all food IDs', error as Error, {
            foodOrigin,
         });
      }
   }
   ...
   async getAllFoodData(
      ...
      try {
         ...
         if (foodOrigin && foodOrigin != undefined && foodOrigin != null) query.where('fn.foodOrigin', foodOrigin);
         const results = await query;
         if (!results || results.length === 0) {
            throw new FoodRepositoryNotFoundException('No food data found', new Error(''), {
               foodOrigin,
               paginated,
            });
         }
         ...
         throw new FoodRepositoryError('Failed to retrieve all food data', error as Error, {
            foodOrigin,
            paginated,
         });
      }
