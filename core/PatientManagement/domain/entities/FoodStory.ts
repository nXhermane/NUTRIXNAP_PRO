...
} from "./../value-objects/WaterConsumptionRange";
...
export class FoodStory extends Entity<IFoodStory> {
    ...
    get favoriteFoods(): IFavoriteFood[] {
        return this.props.favoriteFoods.map((favFood: FavoriteFood) =>
            favFood.unpack()
        );
    }
    get foodAversions(): IAversion[] {
        return this.props.foodAversions.map((foodAve: Aversion) =>
            foodAve.unpack()
        );
    }
    ...
    addDietType(dietTypeId: AggregateID): void {
        if (!this.props.dietTypes.has(dietTypeId))
            this.props.dietTypes.add(dietTypeId);
    }
    removeDietType(dietTypeId: AggregateID): void {
        if (this.props.dietTypes.has(dietTypeId))
            this.props.dietTypes.delete(dietTypeId);
    }
    ...
    removeFavoriteFood(favoriteFood: FavoriteFood): void {
        const favoriteFoodIndex =
            this.props.favoriteFoods.indexOf(favoriteFood);
        if (favoriteFoodIndex !== -1)
            this.props.favoriteFoods.splice(favoriteFoodIndex, 1);
    }
    ...
    removeFoodAversion(foodAversion: Aversion): void {
        const foodAversionIndex =
            this.props.foodAversions.indexOf(foodAversion);
        if (foodAversionIndex !== -1)
            this.props.foodAversions.splice(foodAversionIndex, 1);
    }
    addAllergy(allergyId: AggregateID): void {
        if (!this.props.allergies.has(allergyId))
            this.props.allergies.add(allergyId);
        this.validate();
    }
    removeAllergy(allergyId: AggregateID): void {
        if (this.props.allergies.has(allergyId))
            this.props.allergies.delete(allergyId);
    }
    addFoodIntolerance(foodIntolerance: AggregateID): void {
        if (!this.props.foodIntolerances.has(foodIntolerance))
            this.props.foodIntolerances.add(foodIntolerance);
        this.validate();
    }
    removeFoodIntolerance(foodIntolerance: AggregateID): void {
        if (this.props.foodIntolerances.has(foodIntolerance))
            this.props.foodIntolerances.delete(foodIntolerance);
    }
    addNutritionalDeficience(nutritinalDeficienceId: AggregateID): void {
        if (!this.props.nutritionalDeficiencies.has(nutritinalDeficienceId))
    ...
    validateDietType(dietTypeIds: AggregateID[]): void {
        ...
        for (const dietTypeId of this.props.dietTypes) {
            if (!dietTypeIds.includes(dietTypeId))
                throw new InvalidReference(
                    `La réference ${dietTypeId} au type de regime est invalide`
                );
        }
        this._isValid = true;
    }
    validateAllergy(allergyIds: AggregateID[]): void {
        ...
        for (const allergyId of this.props.allergies) {
            if (!allergyIds.includes(allergyId))
                throw new InvalidReference(
                    `La réference ${allergyId} a l'allergie est invalide`
                );
        }
        this._isValid = true;
    }
    validateFoodIntolerance(foodIntoleranceIds: AggregateID[]): void {
        ...
        for (const foodIntoleranceId of this.props.foodIntolerances) {
            if (!foodIntoleranceIds.includes(foodIntoleranceId))
                throw new InvalidReference(
                    `La réference ${foodIntoleranceId} a l'intolerance alimentaire est invalide`
                );
        }
        this._isValid = true;
    }
    validateNutritionalDeficiencie(nutrientIds: AggregateID[]): void {
        ...
        for (const nutrientId of this.props.nutritionalDeficiencies) {
            if (!nutrientIds.includes(nutrientId))
                throw new InvalidReference(
                    `La réference ${nutrientId} au nutrinent en deficit est invalide`
                );
        }
        this._isValid = true;
    }
    private getFoodIdsOfAversions(): AggregateID[] {
        return this.props.foodAversions.map(
            (foodAv: Aversion) => foodAv.unpack().foodId
        );
    }
    private getFoodIdsOfFavoriteFoods(): AggregateID[] {
        return this.props.favoriteFoods.map(
        ...
        for (const aversionId of aversionFoodIds) {
            if (favoriteFoodIds.includes(aversionId))
                throw new ArgumentInvalidException(
                    `Un aliment ne doit être a la fois une aversion et un aliment préferé.`
                );
        }
        this._isValid = true;
    }
