...
export default class NGramIndex<T> {
    ...
    private getKeys(keys?: string | string[]): string[] {
        if (isDefined(keys)) {
            return isArray(keys)
                ? (keys as string[]).map(
                      (key: string) => `${this._defaultKey}.${key}`
                  )
                : [`${this._defaultKey}.${keys as string}`];
        } else {
    ...
    private sortResult(data: DataTypeInside<T>[]): DataTypeInside<T>[] {
        return data.sort(
            (a: DataTypeInside<T>, b: DataTypeInside<T>) =>
    ...
    private areStringsDisimilar(relevance: number, maxLength: number): boolean {
        return (
            relevance * 100 > Math.floor(maxLength * this._options.threshold)
        );
    }
    ...
    indexDoc(doc: T): void {
        ...
        for (const key of this._keys) {
            ...
            const normalizedText = this._options.isCaseSensitive
                ? text.toLowerCase()
                : text;
    ...
    search(
        pattern: string,
        validate = (doc: DataTypeInside<T>) => true
    ): DataTypeInside<T>[] {
        const normalizedPattern = this._options.isCaseSensitive
            ? pattern.toLowerCase()
            : pattern;
        const ngrams = this.generateNGrams(normalizedPattern);
        const results = new Map<number, DataTypeInside<T>>();
        const searcher = this._options.withBitap
            ? new BitapSearch(pattern, this._options?.searchOption)
            : null;
        for (const ngram of ngrams) {
            if (this.index.has(ngram)) {
                ...
                for (const dataIndex of entries) {
                    ...
                    for (const key of this._keys) {
                        ...
                        if (this._options.withBitap) {
                            ...
                            bitTapResult = searcherResult?.isMatch
                                ? searcherResult
                                : null;
                        } else {
                            relevanceArray.push(
                                1 / levenshteinDistance(pattern, text)
                            );
                    ...
                    if (this._options.withBitap) {
                        ...
                        const maxLength = Math.max(
                            Math.min(...keyValueLength),
                            pattern.length
                        );
                        if (
                            this.areStringsDisimilar(relevance, maxLength) &&
                            validate(doc)
                        ) {
    ...
    toJSON(): { index: string; data: string } {
        const indexJSON = JSON.stringify(
            Array.from(this.index.entries()).map(([key, value]) => [
                key,
                Array.from(value)
            ])
        );
    ...
    static reconstructIndex<T>(indexData: any): Map<string, Set<number>> {
        return new Map<string, Set<number>>(
            indexData.map(([key, value]: [string, number[]]) => [
                key,
                new Set(value)
            ])
        );
