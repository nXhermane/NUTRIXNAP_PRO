...
export default class Trie<T extends Value> {
        ...
        if (isDefined(options)) {
            ...
            if (isDefined(this.options?.keys)) {
                if (isArray(this.options.keys)) {
                    this.keys = (this.options.keys! as string[]).map(
                        (key: string) => this.defaultKey + "." + key
                    );
                }
                if (isString(this.options.keys)) {
                    this.keys.push(
                        (this.defaultKey + "." + this.options.keys!) as string
                    );
                }
            } else {
        ...
        if (prefixSearch) {
            for (const key of this.keys) {
                const normalizedText = (
                    getFn(trieNodeValue, key) as string
                ).toLowerCase();
            ...
            const normalizedText = (
                getFn(trieNodeValue, this.keys[0]) as string
            ).toLowerCase();
    ...
    private collectValueRecursiveInTrie(
        node: TrieNode<number>,
        results: TrieResultValue<T>[],
    ...
    searchFuzzi(
        ...
        this.collectFuzziValueRecursiveInTrie(
            this.root,
            "",
            normalizedPattern,
            fuzziResults,
