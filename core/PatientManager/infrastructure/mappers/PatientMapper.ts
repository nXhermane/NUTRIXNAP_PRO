...
export class PatientMapper implements Mapper<Patient, PatientPersistenceType, PatientDto>{
  ...
  toDomain(record: PatientPersistenceType): Patient {
    ...
    const contact = new Contact({ email: new Email(contactData.email), phoneNumber: new PhoneNumber(contactData.phoneNumber) })
    ...
    return new Patient({ id, createdAt, updatedAt, props: { name, contact, address, gender, birthday, occupation, medicalRecordId, images } })
  }
