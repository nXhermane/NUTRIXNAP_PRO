...
export class MedicalRepositoryImplDb implements MedicalRecordRepository {
   ...
   async save(medicalRecord: MedicalRecord, trx?: Knex.Transaction): Promise<void> {
      try {
         ...
         await Promise.all(
            medicalRecordPersistence.objectives.map(async (objective: ObjectivePersistenceType) => await this.saveObjective(objective, trx)),
         );
         await Promise.all(
            medicalRecordPersistence.foodDiaries.map(async (foodDiary: FoodDiaryPersistenceType) => await this.saveFoodDiary(foodDiary, trx)),
         );
      } catch (error) {
   ...
   async getById(medicalRecordId: AggregateID): Promise<MedicalRecord> {
      try {
         const medicalRecordPersistence = {
            ...
            consultationInformation: await this.knex<ConsultationInformationPersistenceType>(this.consultationInformationTableName)
               .select()
            ...
            personalAndSocialStory: await this.knex<PersonalAndSocialStoryPersistenceType>(this.personalAndSocialStoryTableName)
               .select()
               .where({ medicalRecordId })
               .first(),
            patientMeasurement: await this.knex<PatientMeasurementPersistenceType>(this.patientMeasurementTableName)
               .select()
               .where({ medicalRecordId })
               .first(),
   ...
   async delete(medicalRecordId: AggregateID, trx?: Knex.Transaction): Promise<void> {
      try {
         const query = trx
            ? this.knex<MedicalRecordTableType>(this.medicalRecordTableName).transacting(trx)
            : this.knex<MedicalRecordTableType>(this.medicalRecordTableName);
   ...
   private async saveFoodDiary(foodDiary: FoodDiaryPersistenceType, trx?: Knex.Transaction): Promise<void> {
      ...
      const query = trx
         ? this.knex<FoodDiaryPersistenceType>(this.foodDiaryTableName).transacting(trx)
         : this.knex<FoodDiaryPersistenceType>(this.foodDiaryTableName);
   ...
   private async saveConsultationInformation(consultationInformation: ConsultationInformationPersistenceType, trx?: Knex.Transaction): Promise<void> {
      ...
      const query = trx
         ? this.knex<ConsultationInformationPersistenceType>(this.consultationInformationTableName).transacting(trx)
         : this.knex<ConsultationInformationPersistenceType>(this.consultationInformationTableName);
   ...
   private async saveFoodStory(foodStory: FoodStoryPersistenceType, trx?: Knex.Transaction): Promise<void> {
      ...
      const query = trx
         ? this.knex<FoodStoryPersistenceType>(this.foodStoryTableName).transacting(trx)
         : this.knex<FoodStoryPersistenceType>(this.foodStoryTableName);
   ...
   private async saveMedicalStory(medicalStory: MedicalStoryPersistenceType, trx?: Knex.Transaction): Promise<void> {
      ...
      const query = trx
         ? this.knex<MedicalStoryPersistenceType>(this.medicalStoryTableName).transacting(trx)
         : this.knex<MedicalStoryPersistenceType>(this.medicalStoryTableName);
   ...
   private async savePersonalAndSocialStory(personalAndSocialStory: PersonalAndSocialStoryPersistenceType, trx?: Knex.Transaction): Promise<void> {
      ...
      const query = trx
         ? this.knex<PersonalAndSocialStoryPersistenceType>(this.personalAndSocialStoryTableName).transacting(trx)
         : this.knex<PersonalAndSocialStoryPersistenceType>(this.personalAndSocialStoryTableName);
   ...
   private async savePatientMeasurement(patientMeasurement: PatientMeasurementPersistenceType, trx?: Knex.Transaction): Promise<void> {
      ...
      const query = trx
         ? this.knex<PatientMeasurementPersistenceType>(this.patientMeasurementTableName).transacting(trx)
         : this.knex<PatientMeasurementPersistenceType>(this.patientMeasurementTableName);
   ...
   private async saveObjective(objective: ObjectivePersistenceType, trx?: Knex.Transaction): Promise<void> {
      ...
      const query = trx
         ? this.knex<ObjectivePersistenceType>(this.objectiveTableName).transacting(trx)
         : this.knex<ObjectivePersistenceType>(this.objectiveTableName);
   ...
   private async getConsultationInformationById(consultationInformationId: AggregateID): Promise<ConsultationInformationPersistenceType | undefined> {
      return await this.knex<ConsultationInformationPersistenceType>(this.consultationInformationTableName)
         .select()
         .where('id', consultationInformationId)
         .first();
   }
   ...
   private async getPersonalAndSocialStoryById(personalAndSocialStoryId: AggregateID): Promise<PersonalAndSocialStoryPersistenceType | undefined> {
      return await this.knex<PersonalAndSocialStoryPersistenceType>(this.personalAndSocialStoryTableName)
         .select()
         .where('id', personalAndSocialStoryId)
         .first();
   }
