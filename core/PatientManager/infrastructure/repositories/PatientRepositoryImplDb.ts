...
export class PatientRepositoryImplDb implements PatientRepository {
  ...
  constructor(private knex: Knex, private mapper: Mapper<Patient, PatientPersistenceType, PatientDto>) { }
  async save(patient: Patient, trx?: Knex.Transaction): Promise<void> {
    try {
      ...
      const query = trx ? this.knex<PatientPersistenceType>(this.patientTableName).transacting(trx) : this.knex<PatientPersistenceType>(this.patientTableName)
  ...
  async getAll(paginated?: Paginated): Promise<Patient[]> {
    try {
      ...
      if (patients.length === 0) {
        throw new PatientRepositoryNotFoundException(
