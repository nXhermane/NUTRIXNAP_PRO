...
export class MeasurementTypeRepositoryImplDb implements MeasurementTypeRepository {
  ...
  async save(measuementType: MeasurementType): Promise<void> {
    try {
      ...
      if (!exist)
        await this.knex<MeasurementTypePersistenceType>(this.measurementTypeTableName).insert(measurementTypePersistence)
      else
        await this.knex<MeasurementTypePersistenceType>(this.measurementTypeTableName).where("id", measurementTypePersistence.id).update(measurementTypePersistence)
    } catch (error) {
  ...
  async getById(measurementTypeId: AggregateID): Promise<MeasurementType> {
    try {
      const measurementType = await this.knex<MeasurementTypePersistenceType>(this.measurementTypeTableName).select().where('id', measurementTypeId).first()
      if (!measurementType) throw new MeasurementTypeRepositoryNotFoundException("Type de mesure non trouvée pour l'ID donné", new Error(""), { measurementTypeId })
  ...
  private mapToDomain(measurementTypePersistence: MeasurementTypePersistenceType): MeasurementType {
    ...
    return new MeasurementType({ id, createdAt, updatedAt, props: { name, unit, measureCategory: measureCategory as PatientMeasurementCategory, code: measureCode, nameTranslate: { inFrench: nameF, } } })
  }
}