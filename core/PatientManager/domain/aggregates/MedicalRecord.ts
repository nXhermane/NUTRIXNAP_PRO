...
import {
    ...
    IConsultationInformation
} from "./../entities/ConsultationInformation";
...
import {
    ...
    IPersonalAndSocialStory
} from "./../entities/PersonalAndSocialStory";
import {
    PatientMeasurements,
    IPatientMeasurements
} from "./../entities/PatientMeasurements";
import {
    EatingBehavior,
    IEatingBehavior
} from "./../value-objects/EatingBehavior";
import {
...
export class MedicalRecord extends AggregateRoot<IMedicalRecord> {
        ...
        measurements.forEach(
            (
                ...
                if (measurement instanceof AnthropometricMeasurement) {
                    this.props.measure.addAnthropometricMeasurement(
                        measurement
                    );
                    ...
                    this.props.measure.addBodyCompositionMeasurement(
                        measurement
                    );
                } else {
    ...
    addFoodDiary(...foodDiares: FoodDiary[]) {
        for (const foodDiary of foodDiares)
            this.props.foodDiaries.push(foodDiary);
    }
    ...
    addEatingBehavior(...eatingBehaviors: EatingBehavior[]) {
        for (const eatingBehavior of eatingBehaviors)
            this.props.eatingBehaviors.push(eatingBehavior);
    }
    getMeasurement(
        ...
        | IMedicalAnalysisResult
        | undefined {
        const resultAnthro = this.props.measure.getAnthropometricMesurements(
            measureTypeId,
            last
        );
        if (resultAnthro != undefined) return resultAnthro;
        const resultMedical = this.props.measure.getMedicalAnalysisResults(
            measureTypeId,
            last
        );
        if (resultMedical != undefined) return resultMedical;
        const resultBodyComp =
            this.props.measure.getBodyCompositionMeasurements(
                measureTypeId,
                last
            );
    ...
    getFoodDiary(foodDiaryId: AggregateID): IFoodDiary & BaseEntityProps {
        return this.props.foodDiaries
            .find((foodDiary: FoodDiary) => foodDiary.id == foodDiaryId)
            ?.getProps() as IFoodDiary & BaseEntityProps;
    }
    ...
    getEatingBehavior(): IEatingBehavior[] {
        return this.props.eatingBehaviors.map(
            (eatingBehavior: EatingBehavior) => eatingBehavior.unpack()
        );
    }
    updateMeasure(patientMesurements: PatientMeasurements) {
        if (patientMesurements.equals(this.props.measure))
            this.props.measure = patientMesurements;
    }
    updateConsultationInformation(consultationInfo: ConsultationInformation) {
        if (consultationInfo.equals(this.props.consultationInformation))
            this.props.consultationInformation = consultationInfo;
    }
    updateFoodStory(foodStory: FoodStory): void {
        if (foodStory.equals(this.props.foodStory))
            this.props.foodStory = foodStory;
    }
    updateMedicalStory(medicalStory: MedicalStory): void {
        if (medicalStory.equals(this.props.medicalStory))
            this.props.medicalStory = medicalStory;
    }
    updatePersonalAndSocialStory(
        personalAndSocialStory: PersonalAndSocialStory
    ) {
        if (personalAndSocialStory.equals(this.props.personalAndSocialStory))
            this.props.personalAndSocialStory = personalAndSocialStory;
    }
    updateObjective(...objectives: Objective[]) {
        objectives.forEach((objective: Objective) => {
            const indexOfObjective = this.props.objectives.findIndex(
                (obj: Objective) => obj.equals(objective)
            );
    ...
    updateFoodDiary(...foodDiaries: FoodDiary[]): void {
        foodDiaries.forEach((foodDiary: FoodDiary) => {
            const indexOfFoodDiary = this.props.foodDiaries.findIndex(
                (foodD: FoodDiary) => foodD.equals(foodDiary)
            );
            if (indexOfFoodDiary !== -1)
                this.props.foodDiaries[indexOfFoodDiary] = foodDiary;
        });
    }

    get foodDiaries(): (BaseEntityProps & IFoodDiary)[] {
        return this.props.foodDiaries.map((foodDiary: FoodDiary) =>
            foodDiary.getProps()
        );
    }
    ...
    removeFoodDiary(foodDiaryId: AggregateID): void {
        const indexOfFoodDiary = this.props.foodDiaries.findIndex(
            (foodDiary: FoodDiary) => foodDiary.id == foodDiaryId
        );
        if (indexOfFoodDiary)
            this.props.foodDiaries.splice(indexOfFoodDiary, 1);
    }
    removeObjective(objectiveId: AggregateID): void {
        const indexOfObjective = this.props.objectives.findIndex(
            (obj: Objective) => obj.id == objectiveId
        );
