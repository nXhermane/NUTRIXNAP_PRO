...
export class Objective extends Entity<IObjective> {
   ...
   set type(type: ObjectiveType) {
      ...
      if (this.isMeasure()) {
         ...
         if (Guard.isEmpty(this.props.measureTypeId)) this.props.measureTypeId = 2;
      }
      this.validate();
   }
   ...
   validate(): void {
      if (Guard.isEmpty(this.props.type)) throw new ArgumentNotProvidedException("Le type d'objective doit etre fournit.");
      if (this.props.type === ObjectiveType.Measure) {
         const isValidMeasureObjective = !Guard.isEmpty(this.props.value) || !Guard.isEmpty(this.props.measureTypeId);
         ...
         const isValidGeneralObjectie =
            !Guard.isEmpty(this.props.description) && Guard.isEmpty(this.props.value) && Guard.isEmpty(this.props.measureTypeId);
