...
export class MeasurementType extends Entity<IMeasurementType> {
   ...
   validate(): void {
      if (Guard.isEmpty(this.props.name)) throw new EmptyStringError('Le nom du tyoe de mesure ne peut etre vide');
      if (Guard.isEmpty(this.props.unit)) throw new EmptyStringError("L'unité du type de mesure ne peut etre vide");
