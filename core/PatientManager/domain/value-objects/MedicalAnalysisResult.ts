...
export class MedicalAnalysisResult extends ValueObject<IMedicalAnalysisResult> {
    ...
    validateMeasure(
        ...
        if (
            ...
            PatientMeasurementCategory.MedicalAnalysis
        )
            throw new ArgumentInvalidException(
                "Le type de mesure ne peut etre prise en charge par l'analyse Medicale."
            );
    }
    validate(props: IMedicalAnalysisResult): void {
        ...
        if (props.value < 0)
            throw new NegativeValueError(
                "La valeur d'une mesure issure d'une analyse medicale ne peut etre inferieur à zéro."
            );
        if (Guard.isEmpty(props.unit))
            throw new EmptyStringError(
                "L'unité du mesure issure d'une analyse medicale doit etre fournir."
            );
    }
}
