...
export class FoodDiaryMealEntry extends ValueObject<IFoodDiaryMealEntry> {
    ...
    get foodItems(): IFoodDiaryFoodItem[] {
        return this.props.foodItems.map((foodItem: FoodDiaryFoodItem) =>
            foodItem.unpack()
        );
    }
    ...
    protected validate(props: IFoodDiaryMealEntry): void {
        if (Guard.isEmpty(props.description) && props.foodItems.length === 0) {
            throw new ArgumentInvalidException(
                "Il faut fournir au moins un aliment si la description du repas n'est pas fournir"
            );
        }
        if (Guard.isEmpty(props.mealTypeId)) {
            throw new ArgumentNotProvidedException(
                "Le type de repas doit etre renseigner."
            );
        }
    }
